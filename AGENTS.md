...
- **Minimal API endpoints must stay thin**: Endpoint handlers are only responsible for receiving requests, validating boundary input, and passing work to services. Do not place business logic in endpoint handlers, and avoid endpoint-internal concerns such as detailed logging, `Stopwatch` timing, or workflow orchestration unless there is a clear framework-level requirement. Move business logic to the service layer to keep endpoints lightweight and make behavior easier to test.
- **Projects must follow layered folders**: Application code must be organized into `Endpoints`、`Services`、`Repositories`、`Clients` and other clearly named folders by responsibility. Do not place endpoint, service, repository, or client implementation classes inside `Program.cs`.
- **Program.cs must stay as composition root only**: `Program.cs` is only responsible for service registration, middleware / pipeline setup, configuration wiring, app startup, and mapping endpoint registration entry points. Do not place endpoint handler implementation, business logic, data access logic, or external API calling logic in `Program.cs`.
...
- `Endpoints`：放 Minimal API 路由對應與 endpoint mapping，負責 HTTP request / response、模型繫結、邊界驗證與呼叫 service，不可放商業邏輯。
- `Services`：放真正執行流程的服務類別，集中業務流程協調與主要應用邏輯。
- `Services/Models`：放只給 service 內部使用的資料模型，避免內部流程資料外漏成公開契約。
- `Repositories`：放資料存取邏輯，負責與資料庫、快取或持久化媒介互動，不可承擔 endpoint 或 service 的責任。
...
- `Program.cs`：只保留 DI 註冊、middleware、組態、啟動流程與 endpoint registration 呼叫，不可直接寫 endpoint handler、service、repository、client 的實作。
- 原則總結：入口在 `Endpoints`，商業流程在 `Services`，資料存取在 `Repositories`，外部整合在 `Clients`，`Program.cs` 只負責組裝。
